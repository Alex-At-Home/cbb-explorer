...
object ExtractorUtils {
  ...
  def single_error_enricher(field: String): ParseError => List[ParseError] =
    ParseUtils.enrich_sub_error(`parent_fills_in`, field) _
  /** Enriches errors generated by child functions (multiple errors) */
  def multi_error_enricher(field: String): List[ParseError] => List[ParseError] =
    ParseUtils.enrich_sub_errors(`parent_fills_in`, field) _
  ...
  : Either[List[ParseError], T] =
  {
    extractor.extract(el) match {
      case Some(result) =>
        extractor.build(result).left.map(single_error_enricher(fieldname))
      case None if extractor.fallback.isDefined =>
        Right(extractor.fallback.get)
      case None =>
        Left(List(ParseUtils.build_sub_error(fieldname)(
