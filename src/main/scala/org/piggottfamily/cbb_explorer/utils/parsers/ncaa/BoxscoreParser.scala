...
trait BoxscoreParser {
  ...
  protected object v0_builders extends base_builders {
    ...
    def score_finder(
        doc: Document
    ): List[String] = // 2020+ is 40%, 2019- is 50%
      (doc >?> elementList(
        "div#contentarea table.mytable[width~=[45]0%] td[align=right]"
      ))
        .getOrElse(Nil)
        .map(_.text)
    ...
    def boxscore_finder(
        doc: Document,
        target_team_first: Boolean
    ): Option[List[Element]] = target_team_first match {
      case true =>
        (doc >?> elementList(
      ...
      case false =>
        (doc >?> elementList(
          "div#contentarea br + table.mytable[width=1000px] td a[href]"
        )).filter(_.nonEmpty)
    }
  }
  protected object v1_builders extends base_builders {
    ...
    def score_finder(doc: Document): List[String] =
      (doc >?> elementList(
        "div.table-responsive td[style~=font-size:36px]:matchesOwn([0-9]+)"
      ))
        .getOrElse(Nil)
        .map(_.text)
    ...
    def boxscore_finder(
      ...
      (doc >?> elementList("table.dataTable"))
        .map(
          _.drop(
  ...
  /** Gets the boxscore lineup from the HTML page (external roster has either
    * just names or names + numbers)
    */
  def get_box_lineup(
      filename: String,
      in: String,
      team_id: TeamId,
      format_version: Int,
      external_roster: (List[String], List[RosterEntry]) = (Nil, Nil),
      neutral_game_dates: Set[String] = Set()
    ...
    val doc_request_builder =
      ParseUtils.build_request[Document](`ncaa.parse_boxscore`, filename) _
    val single_error_completer =
      ParseUtils.enrich_sub_error(`ncaa.parse_boxscore`, filename) _

    for {
      doc <- doc_request_builder(browser.parseString(in))

      period <- parse_period_from_filename(filename).left.map(
      ...
      year = Year(
        if (date.monthOfYear.get >= 6) date.year.get else (date.year.get - 1)
      )
      ...
      location_type = maybe_date_str
        .map(_.split(" ")(0)) match { // (get rid of optiomnal time)
        case Some(date_str) if neutral_game_dates.contains(date_str) =>
          Game.LocationType.Neutral
      ...
      val temp_box_score = LineupEvent( // (need this to build a player context)
        date,
        location_type,
        start_min = start_time_from_period(
          period,
          is_women_game = false
      ...
      ordered_lineup = inject_validated_players(
  ...
  /** Validates box players against the roster (if available) and any other
    * available box scores
    */
  def inject_validated_players(
      ordered_lineup_from_box: List[String],
    ...
    val manual_extra_players =
      DataQualityIssues.players_missing_from_boxscore
        .getOrElse(box_minus_players.team.team, Map())
        .getOrElse(box_minus_players.team.year, List())
    ...
    val ordered_lineup = {
      ...
      validated_ordered_lineup ++
        (just_players ++ other_players ++ manual_extra_players)
          .filterNot(validated_ordered_lineup_set)
          .toSet
  ...
  protected def parse_period_from_filename(
      filename: String
    ...
    date
      ...
      .map { date_str =>
        Try(
          ...
          Right(
            formatter
              .parseDateTime(date_str.split(" ")(0))
  ...
  protected def parse_final_score(
      scores_per_period: List[String],
      target_team_first: Boolean
  ...
  protected def parse_players_from_boxscore(
      boxscore_table: Option[List[Element]]
  ...
  protected def validate_box_score(
      team: TeamId,
      lineup: List[String]
