...
object DataQualityIssues {
  ...
  val players_missing_from_boxscore: Map[TeamId, Map[Year, List[String]]] = Map(
    ...
    TeamId("St. Bonaventure") -> Map( // A10
      Year(2023) -> List("Essamvous, Assa")
    )
  )
...
//TODO: will make the history view in hoop-explorer not work
  ...
  val misspellings: Map[Option[TeamId], Map[String, String]] =
    Map( // pairs - full name in box score, and also name for PbP
      ...
      Option(TeamId("Maryland")) -> Map( // (Big Ten)
        (
          // Confusion over name
      ...
      Option(TeamId("Oral Roberts")) -> Map(
        ( // Summit
          // Lots of box scores has him by this nickname (Josh "Colon" Navarro), as do PbP
      ...
      Option(TeamId("Cincinnati")) -> Map(
        (
          ...
          Seq(
            "CUMBERLAND,J" -> "Cumberland, Jarron"
          ) ++ // (some legacy typo, just in case!)
  ...
  /** Generate the 3 different strings: box, PbP, legacy PbP - for use in alias
    ...
    */
  def alias_combos(
      first_last_to: ((String, String), String)
  ): Seq[(String, String)] = {
