...
object LineupErrorAnalysisUtils {
  ...
  /** Context for tidying players, includes a cache for performance if you want
    * to fold with it
    */
  ...
  private def truncate_code_1(code: String): String = {
    val truncation_regex =
      "^([A-Z]).*?([A-Z][a-z.-]*)$".r // (initial then last name)
    code match {
  ...
  private def truncate_code_2(code: String): String = {
    val truncation_regex =
      "^([A-Z][a-z]).*?([A-Z][a-z.-]*)$".r // (initial then last name)
    code match {
  ...
  def build_tidy_player_context(box_lineup: LineupEvent): TidyPlayerContext = {
    ...
    val alt_all_players_map = all_players_map.toList
      .groupBy { case (code, name) =>
        truncate_code_1(code)
      }
      .mapValues(_.map(pp => pp._2)) ++ all_players_map.toList
      .groupBy { case (code, name) =>
  ...
  def tidy_player(
      p_in: String,
      ctx: TidyPlayerContext
  ): (String, TidyPlayerContext) = {
    ctx.resolution_cache.get(p_in).map((_, ctx)).getOrElse {
      val player_id =
        ExtractorUtils.build_player_code(p_in, Some(ctx.box_lineup.team.team))
      ...
      ctx.all_players_map
        ...
        .getOrElse(
          (p, with_updated_cache(p))
        ) // (this will get rejected later on, in validate_lineup)
    }
  }

  /** Find a match for a set of initials */
  def convert_from_initials(
      name: String,
      codes_to_names: Map[String, String]
  ): Option[String] = {
    ExtractorUtils.name_is_initials(name) match {
      case Some((p1, p2)) =>
        val candidates = codes_to_names.collect {
          case (code, id)
              if (code.size >= 3) && (code(0) == p1) && (code(2) == p2) =>
            id
        }
        candidates match {
  ...
  def convert_from_digits(
      name: String,
      player_numbers: List[LineupEvent.PlayerCodeId]
  ): Option[String] = {
  ...
  /** Pulls out inconsistent lineups (self healing seems harder based on cases
    * I've seen, eg player B enters game+player A leaves game ... ... A makes
    * shot...player A enters game)
    */
  def validate_lineup(
      lineup_event: LineupEvent,
      box_lineup: LineupEvent,
      valid_player_codes: Set[String]
    ...
    val all_players_known = lineup_event.players.forall {
      case LineupEvent.PlayerCodeId(code, _, _) => valid_player_codes(code)
    }
    ...
    val inactive_players_mentioned = lineup_event.raw_game_events
      .flatMap(_.team.toList)
      .collect {
        case EventUtils.ParseAnyPlay(player) if player.toLowerCase != "team" =>
          ExtractorUtils
            .build_player_code(
              tidy_player(player, tidy_ctx)._1,
    ...
    (Set(ValidationError.WrongNumberOfPlayers).filterNot(_ =>
      right_number_of_players
    ) ++
      Set(ValidationError.UnknownPlayers).filterNot(_ => all_players_known) ++
      Set(ValidationError.InactivePlayers).filter(_ =>
        inactive_players_mentioned.nonEmpty
      ) ++
      Set()).filter(allowedErrors) // (terminator)
  }

  /** Wraps a related clump of bad lineup events, plus the first following good
    * event
    */
  protected case class BadLineupClump(
      evs: List[LineupEvent],
      next_good: Option[LineupEvent] = None
  )
  ...
  def clump_bad_lineups(
    ...
    lineup_events
      .foldLeft(List[BadLineupClump]()) {
        case (Nil, (lineup, next)) => List(BadLineupClump(lineup :: Nil, next))
        case (
              clumps @ (BadLineupClump(
                clump_evs @ (last :: lineup_tail),
                _
              ) :: clump_tail),
              (lineup, next)
            ) =>
        ...
        case (other, (lineup, next)) =>
          BadLineupClump(
            lineup :: Nil,
            next
          ) :: other // (not possible in practice)
      }
      .map(clump => clump.copy(evs = clump.evs.reverse))
      .reverse
  }

  /** eg 2 in 1 out followed by 1 out next lineup Handles the cases "IN: X, Y,
    * Z; OUT: A, B" ... "OUT: C" and similarly "IN: X, Y, Z; OUT: A" ... "OUT:
    * C, Z"
    */
  ...
  private def display_lineup(ev: LineupEvent, prefix: String) {
    println(
      s"$prefix [${ev.team.team}] vs [${ev.opponent.team}] ([${ev.location_type}])"
    )
  ...
  protected def add_missing_players(
    ...
    if (players_in.size > 4) { // Nothing to do
      ...
      if (extra_debug && debug) {
        ...
        println(
          s"$debug_prefix CLUMP [-$expected_size_diff] size=[${clump.evs.size}] [?${clump.next_good.nonEmpty}]"
        )
      }
      ...
      val initial_candidates = clump.next_good.toList
        .flatMap(_.players_out)
        .filterNot(
          clump.evs.flatMap(_.players).toSet
        )
        .toSet

      val State(_, players_to_add, _) =
        related_evs.foldLeft(State(candidates, initial_candidates, None)) {
          case (State(curr_candidates, curr_to_add, _), ev) =>
            // If you get subbed in, you cease to become a candidate
            val new_candidates =
              curr_candidates -- ev.players_in.filter(curr_candidates)
            // Any candidates who are mentioned get added to the list:
            val candidates_who_are_in_plays = ev.raw_game_events
              .flatMap(_.team.toList)
              .collect { case EventUtils.ParseAnyPlay(player) =>
            ...
            if (extra_debug && debug) {
              ...
              println(s"$debug_prefix candidates=[${curr_candidates
                  .map(_.code)}] > [${new_candidates.map(_.code)}]: found [${candidates_who_are_in_plays
                  .map(_.code)}] = total [${new_curr_to_add.map(_.code)}]")
            }

            State(new_candidates, new_curr_to_add, Some(ev))
        }
      if (players_to_add.nonEmpty) {
        val (good_lineups, bad_lineups) = clump.evs
          .map { ev =>
            val new_ev = ev.copy(
  ...
  /** Goes over the events in a clump removing players who could not be the
    * missing sub
    */
  protected def find_missing_subs(
    ...
    if (candidates.size < 6) { // Nothing to do
      ...
      val State(filtered_candidates, _, maybe_matching_index) =
        related_evs.zipWithIndex.foldLeft(State(candidates, None, None)) {
          ...
          case (State(curr_candidates, _, _), (ev, index)) =>
            val candidates_who_sub_out =
              if (clump.evs.headOption.contains(ev)) Set()
              else ev.players_out.filter(curr_candidates)
            ...
            val new_curr_candidates =
              curr_candidates -- candidates_who_sub_out -- candidates_who_are_in_plays
            if (extra_debug && debug) {
              display_lineup(ev, debug_prefix)
              println(s"$debug_prefix candidates=[${curr_candidates
                  .map(_.code)}] - [${candidates_who_sub_out.map(_.code)}] - [${candidates_who_are_in_plays
                  .map(_.code)}] = [${new_curr_candidates.map(_.code)}]")
              if (new_curr_candidates.size == expected_size_diff) {
                println(
                  s"$debug_prefix matched at index[$index], remainder of events will be recalculated from here"
                )
              }
            }
            ...
            State(
              new_curr_candidates,
              Some(ev),
              if (new_curr_candidates.size == expected_size_diff) Some(index)
              else None // (once we match, we change to phase 2 of processing)
            )
        }
      if (
        filtered_candidates.nonEmpty && (filtered_candidates.size <= expected_size_diff)
      ) {
        case class TidyState(last_ev: Option[LineupEvent])
        val tidied_evs = clump.evs.zipWithIndex
          .scanLeft(TidyState(None)) { // Phase 3
            case (TidyState(maybe_last_ev), (ev, index)) => {
              if (maybe_matching_index.exists(index > _)) { // After match, just build using in/out
                val new_ev = ev.copy(players =
                  ExtractorUtils
                    .build_new_player_list(ev, maybe_last_ev.getOrElse(ev))
                )
        ...
        val (good_lineups, bad_lineups) =
          tidied_evs.partition(
            validate_lineup(_, box_lineup, valid_player_codes).isEmpty
          )
  ...
  def analyze_unfixed_clumps(
    ...
    val validation = clump.evs.headOption.map(ev =>
      validate_lineup(ev, box_lineup, valid_player_codes)
    )

    println(s"$debug_prefix --------------------------------------------------")
    println(
      s"$debug_prefix CLUMP [+$expected_size_diff] size=[${clump.evs.size}] [?${clump.next_good.nonEmpty}] [!${validation}]"
    )
    println(s"$debug_prefix valid player codes: [$valid_player_codes]")

    if (validation.exists(_(ValidationError.UnknownPlayers))) {
      val all_unknown = clump.evs
        .flatMap(_.players)
        .filterNot(id_code => valid_player_codes(id_code.code))
        .distinct
    ...
    println(
      s"$debug_prefix Unfixed\n${clump.evs.foreach(ev => display_lineup(ev, debug_prefix))}"
    )
    clump.next_good.foreach(ev =>
      println(s"$debug_prefix (next good)\n${display_lineup(ev, debug_prefix)}")
    )
  }
  ...
  def analyze_and_fix_clumps(
    ...
    Some(clump)
      ...
      .map { case (fixed, to_fix) =>
        val (newly_fixed, still_to_fix) =
          find_missing_subs(to_fix, box_lineup, valid_player_codes)
        (fixed ++ newly_fixed, still_to_fix)
      }
      .map { case (fixed, to_fix) =>
        val (newly_fixed, still_to_fix) =
          add_missing_players(to_fix, box_lineup, valid_player_codes)
        (fixed ++ newly_fixed, still_to_fix)
      }
      .map {
        case (
              fixed,
              to_fix
            ) => // Try this again since add_missing_players can go too far
          val (newly_fixed, still_to_fix) =
            find_missing_subs(to_fix, box_lineup, valid_player_codes)
          (fixed ++ newly_fixed, still_to_fix)
      }
      .map { case (fixed, to_fix) =>
        ...
        (
          fixed.map {
            fixed_lineup => // recalculate the lineup id now that I've removed players
              fixed_lineup.copy(lineup_id =
                ExtractorUtils.build_lineup_id(fixed_lineup.players)
              )
          },
          to_fix
        )
  ...
  /** Given a list of bad lineups, categorizes them into a map of the number of
    * players (5 indicates that there was a wrong player in there) With the
    * values being the number of "clumps" and the total possessions FOR DISPLAY
    * ONLY (can live without tests)
    */
  def categorize_bad_lineups(
    ...
    bad_clumps
      .groupBy(_.evs.headOption.map(_.players.size).getOrElse(0))
      .mapValues { clumps =>
        (
          clumps.size,
          clumps.foldLeft(0) { (acc, clump) =>
            acc + clump.evs.foldLeft(0) { (sub_acc, lineup) =>
              sub_acc + lineup.team_stats.num_possessions
            }
          }
        )
      }
  ...
  object NameFixer {
    ...
    case class NoSurnameMatch(
        box_name: String,
        exact_first_name: Option[String],
        near_first_name: Option[String],
        err: String
    ) extends MatchResult
    case class WeakSurnameMatch(box_name: String, score: Int, info: String)
        extends MatchResult
    case class StrongSurnameMatch(box_name: String, score: Int)
        extends MatchResult
    ...
    /** Given a PbP name and a single box name, determine how close they are to
      * fitting
      */
    def box_aware_compare(
        candidate_in: String,
        box_name_in: String
    ): MatchResult = {
      ...
      val box_name_decomp = box_name.split(
        "\\s*,\\s*",
        2
      ) // (box is always in "surname-set, other-name-set")
      ...
      def decompose_first_names(
        ...
        val box_first_names = box_name_decomp.toList
          .drop(1)
          .headOption
          .map(_.split("[, ]+").toList.filter(remove_jr))
        val maybe_exact_first_name = box_first_names.collect {
          case List(single_first_name) // needs to be strong enough and match
              if single_first_name.length >= min_useful_first_name_len && candidate_frag_set(
                single_first_name
              ) =>
            single_first_name
        }
        val maybe_near_first_name = box_first_names match {
          ...
          case Some(List(single_first_name))
              if single_first_name.length >= min_useful_first_name_len =>
            filtered_frags
              .filter(
                FuzzySearch
                  .weightedRatio(_, single_first_name) >= min_first_name_score
              )
              .headOption
          case _ => None
        }
      ...
      val best_frag_score = candidate_frag_scores
        .filter {
          case (frag, _)
              if (longest_surname_fragment == frag) && (frag.length >= (min_useful_surname_len - 1)) =>
            true
          // (more permissive if they are actually ==)
          case (frag, score)
              if (score > min_surname_score) && (frag.length >= min_useful_surname_len) =>
            true

          case (frag, score)
              if (score > min_surname_score) && (frag.length >= min_useful_surname_len - 1) =>
            // (also more permissive if the first names match)
            val (maybe_exact_first_name, maybe_near_first_name) =
              decompose_first_names(Some(frag));
      ...
      best_frag_score match {
        ...
        case None => // We will record cases where there is a strong first name match
          val (maybe_exact_first_name, maybe_near_first_name) =
            decompose_first_names();
          // (note we don't bother recording the surname strength in this case since empirically they seem a bit random below 60ish)
          NoSurnameMatch(
            box_name_in,
            maybe_exact_first_name,
            maybe_near_first_name,
            s"[$candidate] vs [$box_name]: " +
              s"Failed to find a fragment matching [$longest_surname_fragment], candidates=${candidate_frag_scores
                  .mkString(";")}"
          )
      }
    }
    ...
    /** The top level method for finding a reliable box score name for a
      * mis-spelled PbP name
      */
    def fuzzy_box_match(
        candidate: String,
        unassigned_box_names: List[String],
        team_context: String
      ...
      val init: (
          List[StrongSurnameMatch],
          List[WeakSurnameMatch],
          List[NoSurnameMatch],
          List[NoSurnameMatch]
      ) = (Nil, Nil, Nil, Nil)

      val match_info = matches.foldLeft(init) {
        case (state, p: StrongSurnameMatch) =>
          (p :: state._1, state._2, state._3, state._4)
        case (state, p: WeakSurnameMatch) =>
          (state._1, p :: state._2, state._3, state._4)
        case (state, p: NoSurnameMatch) if p.exact_first_name.nonEmpty =>
          (state._1, state._2, p :: state._3, state._4)
        case (state, p: NoSurnameMatch) =>
          (state._1, state._2, state._3, p :: state._4)
      }
      ...
      def log_info(maybe_result: Option[String], context: String): Unit = {
        ...
        fixes_for_debug.get(key) match {
          ...
          case Some((curr_result, curr_context, false))
              if result != curr_result =>
            println(
              s"$prefix: ERROR.X: [$result] vs [$curr_result] ([$context] vs [$curr_context]) (key=[$key], box=[$unassigned_box_names])"
            )
            fixes_for_debug =
              fixes_for_debug + (key -> (curr_result, context, true)) // (overwrite so only display this error once)
          case Some((`result`, _, _)) => // nothing to do
          case _ =>
            println(
              s"$prefix: [$result] [$context] (key=[$key], box=[$unassigned_box_names])"
            )
      ...
      match_info match {
          ...
          if (other_strong.nonEmpty) {
            ...
            sorted_l match {
              case best_strong :: less_good_strong =>
                ...
                val new_candidates =
                  less_good_strong.filter(_.score > threshold_score)

                if (new_candidates.nonEmpty) {
                  val context_string =
                    s"ERROR.1A: multiple strong matches: [$candidate] vs [$sorted_l]"
                  log_info(None, context_string)
                  Left(context_string)
                } else {
                  val context_string =
                    s"SUCCESS.1B: multiple strong matches, but clear winner: [$candidate] vs [$sorted_l]"
        ...
        case (Nil, l @ (weak :: other_weak), _, _) =>
          ...
          if (other_weak.nonEmpty) {
            val context_string =
              s"ERROR.2A: multiple weak matches: [$candidate] vs [$l]"
            log_info(None, context_string)
            Left(context_string)
          } else {
        ...
        case (Nil, Nil, l @ (first_name_only :: other_first_name_only), l2) =>
          if (other_first_name_only.nonEmpty) {
            val context_string =
              s"ERROR.3A: multiple first name matches: [$candidate] vs [$l]"
            log_info(None, context_string)
            Left(context_string)
            ...
            val context_string =
              s"ERROR.3B: multiple near first name matches: [$candidate] vs [$bad_l2]"
            log_info(None, context_string)
            Left(context_string)
            ...
            val context_string =
              s"ERROR.3C: 'first name only' match: [$first_name_only]"
