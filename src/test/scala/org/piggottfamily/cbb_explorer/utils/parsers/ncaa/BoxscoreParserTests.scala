    ...
    "BoxscoreParser" - {
      "get_lineup" - {
        { (1, 0.0) :: (2, 20.0) :: (3, 40.0) :: (4, 45.0) :: Nil }.foreach { case (period, mins) =>
          ...
          // Away
          TestUtils.inside(
            get_box_lineup(s"test_p$period.html", lineup_html, TeamId("TeamB"), (Nil, Map()), Set("date_mismatch"))
          ) {
            case Right(LineupEvent(
          ...
          List(lineup_html, adjusted_lineup_html).foreach { html =>
            TestUtils.inside(
              get_box_lineup(s"test_p$period.html", html, TeamId("TeamA"), (Nil, Map()), Set("12/10/2018"))
            ) {
              case Right(LineupEvent(
