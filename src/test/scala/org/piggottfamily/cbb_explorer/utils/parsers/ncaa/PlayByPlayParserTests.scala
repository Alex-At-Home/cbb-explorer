...
object PlayByPlayParserTests extends TestSuite with PlayByPlayParser {
  ...
  val play_by_play_html = Source
    .fromURL(getClass.getResource("/ncaa/test_play_by_play.html"))
    .mkString
    ...
    "PlayByPlayParser" - {
        ...
        TestUtils.inside(
          ...
          TestUtils.inside(lineup_events.drop(1).headOption) {
            case Some(event: LineupEvent) =>
              ...
              event.players
                .map(_.id.name)
                .contains("S6rname, F6rstname TeamA") ==> true
          ...
          lineup_events.zipWithIndex.foreach { event_index =>
            TestUtils.inside(event_index) { case (event, index) =>
              // 0 length lineup
      ...
      "parse_game_events" - {
        TestUtils.inside(
          ...
          game_events.collect { case ev: Model.GameBreakEvent =>
            ev
          }.size ==> 2
          game_events.collect { case ev: Model.GameEndEvent =>
            ev
          }.size ==> 1
        }
      }
      ...
      "enrich_and_reverse_game_events" - {
        ...
        val test_list =
          Model.OtherTeamEvent(2.0, score, "test1") ::
            ...
            Nil
        TestUtils.inside(enrich_and_reverse_game_events(test_list)) { case _ =>
      ...
      "parse_game_score" - {
        TestUtils.with_doc(sample_team_event) { doc =>
          TestUtils.inside(parse_game_score(doc.body, v0_builders)) {
      ...
      "parse_desc_game_time" - {
        TestUtils.with_doc(sample_team_event) { doc =>
          TestUtils.inside(parse_desc_game_time(doc.body, v0_builders)) {
            case Right(("15:00", t)) =>
              "%.1f".format(t) ==> "15.0"
          }
        }
      ...
      "parse_game_event" - {
        TestUtils.with_doc(sample_game_event) { doc =>
          TestUtils.inside(
            parse_game_event(doc.body, target_team_first = true, v0_builders)
          ) { case Right(Nil) =>
          }
        }
        TestUtils.with_doc(sample_team_event) { doc =>
        ...
        TestUtils.with_doc(sample_team_sub_in) { doc =>
          TestUtils.inside(
            ...
            case Right(
                  List(Model.SubInEvent(t, _, "S8RNAME,F8RSTNAME TEAMA"))
                ) =>
              "%.1f".format(t) ==> "15.0"
          }
        }
        TestUtils.with_doc(sample_team_sub_in_new_format) { doc =>
          TestUtils.inside(
            ...
            case Right(
                  List(Model.SubInEvent(t, _, "F5rstname TeamA S5rname"))
                ) =>
              "%.1f".format(t) ==> "15.0"
          }
        }
            ...
            case Right(
                  List(
                    Model.OtherOpponentEvent(
                      t,
                      Game.Score(45, 26),
                      "15:00,45-26,S8RNAME,F8RSTNAME TEAMB Enters Game"
                    )
                  )
                ) =>
              "%.1f".format(t) ==> "15.0"
        ...
        TestUtils.with_doc(sample_team_sub_out) { doc =>
          TestUtils.inside(
            ...
            case Right(
                  List(Model.SubOutEvent(t, _, "S8RNAME,F8RSTNAME TEAMA"))
                ) =>
              "%.1f".format(t) ==> "15.0"
          }
        }
        TestUtils.with_doc(sample_team_sub_out_new_format) { doc =>
          TestUtils.inside(
            ...
            case Right(
                  List(Model.SubOutEvent(t, _, "F5rstname TeamA S5rname"))
                ) =>
              "%.1f".format(t) ==> "15.0"
          }
          TestUtils.inside(
            parse_game_event(doc.body, target_team_first = false, v0_builders)
          ) {
            case Right(
                  List(
                    Model.OtherOpponentEvent(
                      t,
                      Game.Score(26, 45),
                      "15:00,45-26,F5rstname TeamA S5rname, substitution out"
                    )
                  )
                ) =>
              "%.1f".format(t) ==> "15.0"
        ...
        TestUtils.with_doc(sample_oppo_sub_out) { doc =>
            ...
            case Right(
                  List(
                    Model.OtherOpponentEvent(
                      t,
                      Game.Score(45, 26),
                      "15:00,45-26,S8RNAME,F8RSTNAME TEAMB Leaves Game"
                    )
                  )
                ) =>
              "%.1f".format(t) ==> "15.0"
          ...
          TestUtils.inside(
            ...
            case Right(
                  List(Model.SubOutEvent(t, _, "S8RNAME,F8RSTNAME TEAMB"))
                ) =>
              "%.1f".format(t) ==> "15.0"
          }
        }
